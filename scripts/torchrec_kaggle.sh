...
torchx run -s local_cwd -cfg log_dir=log/torchrec_kaggle/w${GPUNUM}_${BATCHSIZE} dist.ddp -j 1x${GPUNUM} --script baselines/dlrm_main.py -- \
    ...
    --learning_rate 1. --batch_size ${BATCHSIZE} --profile_dir "tensorboard_log/torchrec_kaggle/w${GPUNUM}_${BATCHSIZE}" --sharder_type "naive" --eval_acc 2>&1 | tee logs/torchrec_${GPUNUM}_${BATCHSIZE}.txt
# exit(0)
