...
#         x = super(MyEmbedding, self).forward(x.to('cpu'))
#         return x.to(current_dev)
#
#
# class Net(torch.nn.Module):
#     def __init__(self):
#         super(Net, self).__init__()
#         self.embed = MyEmbedding(20, 4)
...
def run_hybrid_device(use_ddp):
    ...
    org_size = real_model.embed.weight.size(0)
    ...
    init_colo_module(real_model.embed, parallel_action, recursive=True, mode='row')
    ...
    assert real_model.embed.weight.size(0) == org_size // gpc.get_world_size(ParallelMode.PARALLEL_1D)
