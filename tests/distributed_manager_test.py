...
def all_reduce(use_cpu):
    ...
    local_tensor = torch.tensor([dist_manager.get_rank() * SIZE + j for j in range(SIZE)]).to(device)
    buffer = local_tensor.clone()
    print(f"Rank: {dist_manager.get_rank()}, before: {local_tensor}")

    dist.all_reduce(buffer, group=(dist_manager.get_cpu_group() if use_cpu else dist_manager.get_group()))
    print(f"Rank: {dist_manager.get_rank()}, after: {buffer}")


def run(rank, world_size, port, use_cpu):
    disable_existing_loggers()
    launch(rank=rank, world_size=world_size, port=port, host='localhost', backend='nccl')
    assert dist_manager.get_rank() == dist.get_rank()
    assert dist_manager.get_world_size() == dist.get_world_size()
    print(f"Rank: {dist_manager.get_rank()}, world size: {dist_manager.get_world_size()}")
