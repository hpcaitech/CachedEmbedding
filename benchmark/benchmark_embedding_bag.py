...
def benchmark(num_embeddings, embedding_dim, batch_size):
    ...
    print(f'batch_size {batch_size} embedding_dim {embedding_dim} num_embeddings {num_embeddings} cuda/cpu {cuda_timer.elapsed / cpu_timer.elapsed}, cpu time {cpu_timer.elapsed}, cuda time {cuda_timer.elapsed}')

if __name__ == '__main__':
    for batch_size in [4, 16, 32]:
        for embedding_dim in [128, 1024, 2048]:
            for num_embeddings in [1000, 10000, 100000]:
